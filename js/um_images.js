...
function resizeImgContainer(container, image){
	...
	if (maxWidth <= 1) maxWidth *= jQuery(window).width();
	if (maxHeight <= 1) maxHeight *= jQuery(window).height();
...
function setBigImage(bigImage, srcType, source, animate){
	...
	switch (srcType){
		case 'img':
			if (jQuery(source).is('img') && jQuery(source).attr('data-big-image')){
	...
	jQuery(bufferImg).on('load', function(){
		...
		if (!animate){
			...
			bigImage.attr({'src': src, 'srcset': src, 'alt': alt});
			bufferImg = null;
		} else {
			...
			bigImage.fadeOut(300, function(){
				resizeImgContainer(parent, bufferImg);
				jQuery(this).attr({'src': src, 'srcset': src, 'alt': alt});
			}).fadeIn(300, function(){
	...
	jQuery(bufferImg).attr({'src': src, 'srcset': src, 'alt': alt});
}
