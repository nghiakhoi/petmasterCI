...
function umPopupToggle(popupSwitch, hide){
	...
	var container = jQuery(popupSwitch.attr('data-popup-container'));
...
function umPopupIsClicked(popupSwitch, target){
	var content = jQuery(popupSwitch.attr('data-popup-container')).find('.um_popup-content');
...
function popupReposition(popupSwitch, target){
	var popup = jQuery(popupSwitch.attr('data-popup-container'));
...
function closePopups(closeFast){
	if (closeFast){
		jQuery('.um_popup-switch.um_opened').each(function(){
			umPopupToggle(jQuery(this), 'hide');
		});
	}
	else{
		jQuery('.um_popup-switch.um_opened').each(function(){
			umPopupToggle(jQuery(this));
		});
	}
	jQuery('body').removeClass('darken');
}
...
jQuery(function($){
	...
	// opened that panel
	jQuery('.um_popup-content').on('click', function(e){
		e.stopPropagation();
	})
	...
	jQuery('.order-number-link').on('click', function(e){
		e.preventDefault();
		if (jQuery(this).hasClass('um_opened')){
			umPopupToggle(jQuery(this));
			return;
		} else {
			closePopups(true);
	...
	jQuery('.basket-toggle-btn').on('click', function(e){
		/* BACK_END not need while not popup small basket
		e.preventDefault();
	...
	jQuery( document ).on('click', function(e){
		...
		if (jQuery(target).hasClass('um_popup-switch') || 
			jQuery(target).parents('.um_popup-switch').length != 0) return;
		...
		var openedPopups = jQuery('.um_popup-switch.um_opened');
		openedPopups.each(function(){
			if (!umPopupIsClicked(jQuery(this),target)) {
				closePopups();
			}
		});
	});
